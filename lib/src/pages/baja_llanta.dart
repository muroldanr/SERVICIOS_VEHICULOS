...
class BajarLLanta extends StatefulWidget {
  final String id;
  ...
  @override
  _BajarLLantaState createState() => _BajarLLantaState(
      this.id,
      this.descripcion1,
      this.serie,
      this.posicion,
...
class _BajarLLantaState extends State<BajarLLanta> {
  final String id;
  final String descripcion1;
  final String serie;
  final String posicion;
      ...
      child: DaScaffoldLoading(
        ...
        children: <Widget>[
          Scaffold(
              ...
              appBar: NavBarCustom("Bajar Neumático"),
  ...
  _getDetalle() async {
    final httpProv = new HttpProvider();
    var res = await (httpProv.spWebLlantaInfo(id, posicion) as FutureOr<List<LlantaDetalle>>);
    if (res[0] != null) {
      setState(() {
        detalle = res[0];
      });
  ...
  */
  ...
  _requestBajaLLanta() async {
    ...
    final httpProv = new HttpProvider();
    final List<Response> lista = await (httpProv.spWebBajaLlanta(
      entID,
      entPosicion,
    ...
    if (lista[0].ok == null || lista[0].ok == "null") {
      ...
      _alertTextDesasigna(context, lista[0].okRef.toString());
  ...
  Future<void> _alertTextDesasigna(
      BuildContext context, messageResponse) async {
    return showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
            shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(10.0)),
            title: Center(child: Text(messageResponse)),
            actions: <Widget>[
              new TextButton(
                  onPressed: () {
  ...
  _opcionConAnexo() {
    ...
    Navigator.of(context, rootNavigator: true).pop('dialog');
    setState(() {
      _nameImage = 'Anexo_Llantas ' + today.toString() + " " + hora.toString();
    });
  ...
  _getPahForImage() async {
    ...
    final List<ResponseRuta> res =
        await (httpProv.spWebRutaLlantasAnexos(llanta.toString()) as FutureOr<List<ResponseRuta>>);
  ...
  Future<void> sendImage(context) async {
    ...
    var res = await (httpProv.anexos(
        80006, _ruta, "FAV", _nameImage! + ext, ext, mimeType, img64) as FutureOr<Response>);
